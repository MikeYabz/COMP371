...
//    float randomValue = EventManager::GetRandomFloat(0.0f, averageTimeBetweenEmission);
    ...
    for (std::list<Particle*>::iterator it = mParticleList.begin(); it != mParticleList.end(); it++)
    {
		...
		{
			p->billboard.color = mix(mpDescriptor->midColor, mpDescriptor->endColor, ((p->currentTime - (mpDescriptor->totalLifetime - mpDescriptor->fadeOutTime))/mpDescriptor->fadeOutTime));
		}
